...
class TestStringMethods(unittest.TestCase):
    ...
    def test_punct0(self):
        sentence = "Is this a good/ready Logo?"
        tokens = [t.text for t in SpacyTokens(sentence).remove_all(punct, stop).lower()]
        output = " ".join(tokens)
        expected = "good ready logo"
    ...
    def test_punct2(self):
        ...
        output = " ".join(t.text for t in SpacyTokens(sentence).remove_all(punct, stop).lower())
    ...
    def test_lemmatize1(self):
        ...
        output = " ".join(t.text for t in SpacyTokens(sentence).lemmatize().remove_all(punct, stop).lower())
